...
The dataset covers a limited historical window, so these metrics should not be interpreted as true long-term retention. To reduce right-censoring, the primary comparison uses a fixed 90-day repeat window and only includes customers whose first purchase occurred at least 90 days before the final observed order date.
...
Among first-order categories with at least 300 eligible customers, the highest observed 90-day repeat rate was `home_appliances` at 7.7%.
